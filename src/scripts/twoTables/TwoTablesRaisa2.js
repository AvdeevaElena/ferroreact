...
export class TwoTablesRaisa2 extends React.Component{

  requestData(dataToRequest){
    const self = this;
    const AuthStr =JSON.parse(localStorage.getItem('currentUser'));
    
    RequestTwoTablesData('Раиса2', 'HEAT', 'http://172.16.20.75:8060/?heat_table=raisa2&program_number='+dataToRequest+'&year=2019', AuthStr).then(resultTwoTable=>{
      self.setState({dataTableHeat:   resultTwoTable.dataTable});
    }); 

    RequestTwoTablesData('Раиса2', 'GAS', 'http://172.16.20.75:8060/?gas_table=raisa2&program_number='+dataToRequest+'&year=2019', AuthStr).then(resultTwoTable=>{
...
export default TwoTablesRaisa2;