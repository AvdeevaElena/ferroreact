...
import GraphFR06                           from '../graphs/GraphFR06';
import                                           './info.css';
import                                           '../style.css';
import logo_loading                         from '../../logos/fp_logo_loading.svg';
import { connect }                          from 'react-redux';
import { RequestTimelineData }              from '../receivers/requestData'
import { TimelineColumns, TableColumnsFR06 }    from '../receivers/handleDataResponse';
...
export class InfoFR06        extends React.Component{
  ...
  requestData(){
    ...
    RequestTimelineData(kiln_constants_ru.ФР06, 'http://172.16.20.75:8060/?generaltimeline=' + kiln_constants_en.ФР06, AuthStr)
  ...
  chartEventsTimeline =[{
    ...
    callback  : ({chartWrapper}) => {        
      ...
      this.props.dispatch(burn_graph_number_received(kiln_constants_ru.ФР06, parseInt(selectedTimelineValue))); 
    }
  }];
  ...
  chartEventsTable =[{
    ...
    callback  : ({chartWrapper}) => { 
      ...
      this.props.dispatch(burn_two_tables_number_received(kiln_constants_ru.ФР06, parseInt(selectedTableValue))); 
    }
  }];
  ...
  render(){
    return (
      <div className={'my-global-div'} >
        ...
        <div id='graph' className={'my-graphFR06-div'}>
         { this.state && this.state.dataTimeLine && this.props.aux_data_received &&this.props.aux_data_received.number && 
                                                   !this.props.aux_data_received.two_tables_number &&
                <GraphFR06 programNumber={this.props.aux_data_received.number}/> 
         }
       </div>
      </div>
    );
...
const mapStateToProps = state => ({
  aux_data_received: state.aux_data_received.filter(c => c.kiln==kiln_constants_ru.ФР06)[0]
}) 
...
export default connect(mapStateToProps, mapDispatchToProps)(InfoFR06);